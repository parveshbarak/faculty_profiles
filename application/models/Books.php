...
class Books extends CI_Model
{
   ...
   {
      $query = $this->db->query("SELECT * FROM books WHERE user_id='$code'");
      $result = $query->result();

      return $result;
   }

   public function get_book($code, $id)
   {
      //data is retrive from this query 
      $query = $this->db->query("SELECT * FROM books WHERE user_id='$code' AND id = '$id'");
      $result = $query->result();
      return $result;
   }
   ...
   public function add_book($data, $code, $serial_no,$image_path)
   {
       $query = $this->db->WHERE('user_id',$code);
